...
use crate::Frame;
use crate::rocks::{CF_NAME_SET_DATA, CF_NAME_SET_GC, CF_NAME_SET_META, CF_NAME_SET_SUB_META, gen_next_meta_index, get_client, KEY_ENCODER, Result as RocksResult, tx_scan, tx_scan_cf, tx_scan_keys, tx_scan_keys_cf};
...
impl SetCommand {
    fn sum_key_size(
        ...
        let meta_cf = client.cf_handle(CF_NAME_SET_META)?;
        let sub_meta_cf = client.cf_handle(CF_NAME_SET_SUB_META)?;
        ...
        client.exec_txn(move |txn| {
            ...
            match txn.get_cf(&meta_cf, meta_key)? {
                Some(meta_value) => {
                    ...
                    let iter = tx_scan_cf(txn, &sub_meta_cf, bound_range, u32::MAX)?;
                    let sum = iter
    ...
    pub async fn sadd(self, key: &str, members: &Vec<String>) -> RocksResult<Frame> {
        ...
        let meta_cf = client.cf_handle(CF_NAME_SET_META)?;
        let sub_meta_cf = client.cf_handle(CF_NAME_SET_SUB_META)?;
        let data_cf = client.cf_handle(CF_NAME_SET_DATA)?;
        let key = key.to_owned();
        ...
        let resp = client.exec_txn(move |txn| {
            match txn.get_cf(&meta_cf, meta_key.clone())? {
                Some(meta_value) => {
                    if !matches!(KeyDecoder::decode_key_type(&meta_value), DataType::Set) {
                        return Err(REDIS_WRONG_TYPE_ERR);
                    }
                    ...
                    let cf_key_pairs = member_data_keys.clone().into_iter().map(|k| (&data_cf, k))
                    ...
                    for m in &members {
                        let data_key =
                            KEY_ENCODER.encode_txn_kv_set_data_key(&key, m, version);
                        txn.put_cf(&data_cf, data_key, vec![0])?;
                    }
                    ...
                    let new_sub_meta_value =
                        txn.get_cf(&sub_meta_cf, sub_meta_key.clone())?.map_or_else(
                            || added,
                            |value| {
                    ...
                    txn.put_cf(&sub_meta_cf, sub_meta_key, new_sub_meta_value.to_be_bytes().to_vec())?;

                    // create a new meta key if key already expired above
                    if expired {
                        let new_meta_value =
                            KEY_ENCODER.encode_txn_kv_set_meta_value(0, version, 0);
                        txn.put_cf(&meta_cf, meta_key, new_meta_value)?;
                    }
                ...
                None => {
                    ...
                    txn.put_cf(&meta_cf, meta_key, meta_value)?;
                    ...
                    let sub_meta_key =
                        KEY_ENCODER.encode_txn_kv_sub_meta_key(&key, version, rand_idx);
                    txn.put_cf(&sub_meta_cf, sub_meta_key, added.to_be_bytes().to_vec())?;
                    Ok(added)
                }
            }
        });
    ...
    pub fn txn_expire_if_needed(
        ...
        let sub_meta_cf = client.cf_handle(CF_NAME_SET_SUB_META)?;
        let data_cf = client.cf_handle(CF_NAME_SET_DATA)?;
        let gc_cf = client.cf_handle(CF_NAME_SET_GC)?;
        ...
        match txn.get_cf(&meta_cf, meta_key.clone())? {
            Some(meta_value) => {
                ...
                if size > async_expire_set_threshold_or_default() as i64 {
                    // async del set
                    txn.delete_cf(&meta_cf, meta_key)?;

                    let gc_key = KEY_ENCODER.encode_txn_kv_gc_key(&key);
                    txn.put_cf(&gc_cf, gc_key, version.to_be_bytes())?;
                    ...
                    let iter = tx_scan_keys_cf(txn, &sub_meta_cf, sub_meta_range, u32::MAX)?;
                    for k in iter {
                        txn.delete_cf(&sub_meta_cf, k)?;
                    }

                    let data_bound_range =
                        KEY_ENCODER.encode_txn_kv_set_data_key_range(&key, version);
                    let iter = tx_scan_keys_cf(txn, &data_cf, data_bound_range, u32::MAX)?;
                    for k in iter {
                        txn.delete_cf(&data_cf, k)?;
                    }
